...
// Theme helper (enhanced: clears inline colors and forces readable text)
function applyTheme(theme) {
  // toggle classes
  document.body.classList.remove("dark-mode","sepia-mode","light-mode");
  if (theme === "black") document.body.classList.add("dark-mode");
  ...
  // Notify timer UI (if currently present)
  try { if (typeof window.updateTimerTheme === "function") window.updateTimerTheme(); } catch(e) {}

  // small helper to force readable text and clear inline color styles
  try {
    function forceReadable() {
      // #text area
      var txt = document.getElementById('text');
      if (txt) {
        // try to remove inline color from children so our CSS takes precedence
        txt.style.color = '#f5f5f5';
        var all = txt.getElementsByTagName('*');
        for (var i = 0; i < all.length; i++) {
          // if an element has an inline color, clear it so stylesheet can win
          if (all[i].style && all[i].style.color) {
            all[i].style.color = '';
          }
        }
      }
      ...
      for (var j = 0; j < choices.length; j++) {
        choices[j].style.color = '#f5f5f5';
        var kids = choices[j].getElementsByTagName('*');
        for (var k = 0; k < kids.length; k++) {
          if (kids[k].style && kids[k].style.color) {
            kids[k].style.color = '';
          }
        }
      }

      // #gameContainer fallback
      var gc = document.getElementById('gameContainer');
      if (gc) {
        gc.style.color = '#f5f5f5';
      }
    }

    // force immediately and once after a short delay to handle re-renders
    forceReadable();
    setTimeout(forceReadable, 120);
    // optional extra pass a little later if ChoiceScript still mutates DOM
    setTimeout(forceReadable, 800);
  } catch(e) {
    // fail silently
  }
}
...
(function initSavedTheme() {
  try {
    var saved = window.localStorage.getItem("uiTheme");
...
function showPsychProfile() {
  console.log("showPsychProfile() called");
  if (document.getElementById('loading')) {
    console.log("Loading in progress; returning");
    return;
  }

  var button = document.getElementById("psychProfileButton");
  if (button && button.innerHTML === "Return") {
    console.log("Button says Return to Game; clearing screen and returning");
    return clearScreen(function() {
      setButtonTitles();
      loadAndRestoreGame();
    });
  ...
  clearScreen(function() {
    console.log("Screen cleared, preparing container...");
    ...
    let overlay = document.getElementById("psychProfileOverlay");
    if (!overlay) {
      overlay = document.createElement("div");
      overlay.id = "psychProfileOverlay";
      document.body.appendChild(overlay);
    }

    let popup = document.getElementById("psychProfilePopup");
    if (!popup) {
      popup = document.createElement("div");
      popup.id = "psychProfilePopup";
      overlay.appendChild(popup);
    }

    setButtonTitles();

    var scene = new Scene("choicescript_psychprofile", window.stats, this.nav, {
      secondaryMode: "profile",
      saveSlot: "temp"
    });
    console.log("Created scene. Executing...");

    scene.execute();

    // poll for content loading
    ...
    const checkContentLoaded = setInterval(function() {
      ...
      if (mainDiv && mainDiv.innerHTML.trim() !== "") {
        ...
        let dynamicContent = mainDiv.innerHTML;

        const personality = window.stats["personalitytype_" + window.stats.personalityindex];
        let contentHTML = `
          <div class="header">PSYCHOLOGICAL PROFILE</div>
          ...
          <div class="stat-chart">
            <p>HUMANITY LEVEL: ${window.stats.humanity || "N/A"}</p>
            <p>[STATUS: ${window.stats.humanitystatus || ""}]</p>
          </div>
        `;
        ...
        popup.querySelectorAll('.traits-section').forEach(section => {
          section.innerHTML = section.innerHTML
            .replace(/(★)/g, '<span class="starFilled">$1</span>')
            .replace(/(☆)/g, '<span class="starEmpty">$1</span>');
        });

        popup.classList.add("psychProfileScene");
        var personalityIndex = Number(window.stats.personalityindex);
        var personalityClass = "";
        switch(personalityIndex) {
          case 1: personalityClass = "hardWorkingIdiot"; break;
          case 2: personalityClass = "badassDelinquent"; break;
          case 3: personalityClass = "underdog"; break;
          case 4: personalityClass = "allLovingHero"; break;
          case 5: personalityClass = "theRuthless"; break;
          default: personalityClass = "defaultPersonality";
        }
        popup.classList.add(personalityClass);

        console.log("Personality Index:", personalityIndex);
        console.log("Applied Personality Class:", personalityClass);

        const returnBtn = document.createElement("button");
        returnBtn.classList.add("return-btn");
        returnBtn.textContent = "Return to Game";
        returnBtn.id = "closePsychProfile";
        popup.innerHTML += returnBtn.outerHTML;

        let returnButton = document.getElementById("closePsychProfile");
        returnButton.addEventListener("click", function() {
          console.log("Return to Game clicked");
          ...
          setTimeout(function() {
            ...
            console.log("Game screen restored.");
          }, 300);
        });

        overlay.classList.add("active");
        popup.classList.add("active");
        console.log("Psych profile popup displayed.");
      } else if (attempts >= maxAttempts) {
        clearInterval(checkContentLoaded);
        console.warn("Content loading timed out after max attempts.");
      }
    }, 100); // Check every 100ms
  });
}

/* formatSlotKey returns the plugin-style canonical slot (NO "state" prefix)
   e.g. "CHOICESCRIPT_SAVE_1630000000000" */
function formatSlotKey(id) {
  var store = (typeof window.storeName === "string" && window.storeName.length) ? window.storeName : "CHOICESCRIPT";
  return store + "_SAVE_" + id;
}
...
function cloneStats(stats) {
  const clone = {};
  if (!stats || typeof stats !== "object") return clone;
  for (let key in stats) {
    ...
    if (typeof val === "object" && val !== null) {
      try {
        ...
        // cannot serialize: skip or shallow copy
        try { clone[key] = Object.assign({}, val); } catch (er) { /* skip */ }
      }
    } else {
      clone[key] = val;
    }
...
/* safe accessor for sceneName across different save blob shapes */
function getSceneNameFromBlob(blob) {
  if (!blob) return "";
  // plugin-style: baseSave.stats.sceneName
  if (blob.stats && typeof blob.stats.sceneName === "string") return blob.stats.sceneName;
  // fallback shapes: blob.sceneName or blob.sceneName inside blob
  if (typeof blob.sceneName === "string") return blob.sceneName;
  // older fallbacks
  return "";
}
...
function showSaveSystem() {
  console.log("showSaveSystem() called");
  ...
  // remove any previous overlay duplicates (safe-guard)
  const prev = document.getElementById("saveSystemOverlay");
  if (prev) prev.remove();

  function canonicalSlot(slotName) {
    if (!slotName || typeof slotName !== "string") return slotName;
    return slotName.startsWith("state") ? slotName.slice(5) : slotName;
  }

  const overlay = document.createElement("div");
  overlay.className = "csssp-popup-overlay";
  overlay.id = "saveSystemOverlay";
  overlay.style.zIndex = "9999";
  overlay.style.pointerEvents = "none"; // enabled after a tiny delay

  const wrapper = document.createElement("div");
  wrapper.className = "csssp-popup-window";
  wrapper.id = "saveSystemWrapper";

  const title = document.createElement("h3");
  title.className = "csssp-popup-title";
  title.textContent = "SAVE MENU";
  wrapper.appendChild(title);

  const selectEle = document.createElement("select");
  selectEle.id = "saveSlotMenu";
  selectEle.className = "csssp-popup-select";
  selectEle.setAttribute("aria-label", "Save Slot");
  wrapper.appendChild(selectEle);

  let saves = [];
  try {
    saves = JSON.parse(localStorage.getItem("CS_SaveList") || "[]");
    if (!Array.isArray(saves)) saves = [];
  } catch (e) {
    console.warn("Invalid CS_SaveList JSON, resetting:", e);
    saves = [];
  }
  ...
  function makePlaceholder() {
    ...
    ph.value = "0";
    ph.innerHTML = "Select a Save Slot";
    return ph;
  }

  function populateSelect() {
    selectEle.innerHTML = "";
    selectEle.appendChild(makePlaceholder());
    saves.sort((a, b) => (b.id || b.time || 0) - (a.id || a.time || 0));
    saves.forEach(save => {
      const opt = document.createElement("option");
      opt.value = String(save.id);
      opt.dataset.slot = save.slot ? canonicalSlot(save.slot) : canonicalSlot(formatSlotKey(save.id));
      const displayName = save.name && save.name.length ? save.name : "Save";
      const timeVal = save.time || save.id || 0;
      opt.textContent = `${displayName} — ${new Date(timeVal).toLocaleString()}`;
  ...
  btnRow.className = "csssp-popup-button-row";

  function makeBtn(label, handler) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "csssp-popup-btn";
    btn.textContent = label;
    btn.addEventListener("click", handler);
    return btn;
  }

  function saveMetaList() {
    try { localStorage.setItem("CS_SaveList", JSON.stringify(saves)); } catch (e) { console.error("Failed to write CS_SaveList:", e); }
  }

  function deepCloneSerializable(value, seen = new WeakSet()) {
    ...
    seen.add(value);
    if (value && value.constructor && value.constructor.name === "Scene") return undefined;
    if (Array.isArray(value)) {
      const res = [];
      for (let i = 0; i < value.length; i++) {
        const c = deepCloneSerializable(value[i], seen);
        if (typeof c !== "undefined") res.push(c);
      }
      return res;
    }
    const out = {};
    for (const k in value) {
      ...
      if (k === "scene" || k === "window" || k === "document") continue;
      const v = value[k];
      if (typeof v === "function") continue;
      const cloned = deepCloneSerializable(v, seen);
      if (typeof cloned !== "undefined") out[k] = cloned;
    }
    return out;
  }

  function attemptPluginSave(slotName, statsObj, tempsObj, lineNum, indent, onSuccess, onFailure) {
    if (typeof saveCookie !== "function") {
      return onFailure && onFailure(new Error("saveCookie not available"));
    }
    try {
      // saveCookie(callback, slotName, stats, temps, lineNum, indent)
      saveCookie(function(cbArg) {
        if (typeof cbArg === "boolean" && cbArg === false) return onFailure && onFailure(new Error("saveCookie callback reported failure"));
        return onSuccess && onSuccess();
      }, slotName, statsObj, tempsObj, lineNum, indent);
    } catch (err) {
      return onFailure && onFailure(err);
    }
  }

  // disable re-entrancy while a save/load/delete is in progress
  let inFlight = false;

  // fallback manual save (reads plugin state if possible, otherwise uses window.stats/window.scene)
  function performFallbackManualSave(saveName, done) {
    const id = Date.now();
    const slotName = canonicalSlot(formatSlotKey(id));

    // try to source state via restoreObjec
    if (typeof restoreObject === "function" && typeof initStore === "function") {
      try {
        restoreObject(initStore(), "state", null, function(baseSave) {
          // If restoreObject returns a usable baseSave, use it; otherwise fall back to global state
          let statsSrc = (baseSave && baseSave.stats) ? baseSave.stats : (window.stats || {});
          let tempsSrc = (baseSave && baseSave.temps) ? baseSave.temps : ((window.scene && window.scene.temps) ? window.scene.temps : {});
          // Ensure metadata
          try { if (!statsSrc) statsSrc = {}; statsSrc["_smSaveName"] = saveName || ""; statsSrc["_smSaveDateId"] = id; } catch (e) { console.warn("Failed to set save meta:", e); }

          // build safe blob guy
          try {
            const safeStats = deepCloneSerializable(statsSrc) || {};
            const safeTemps = deepCloneSerializable(tempsSrc) || {};
            if (typeof safeStats.sceneName === "undefined" && typeof statsSrc.sceneName !== "undefined") safeStats.sceneName = statsSrc.sceneName;
            const blob = {
              stats: safeStats,
              temps: safeTemps,
              lineNum: (baseSave && typeof baseSave.lineNum !== "undefined") ? baseSave.lineNum : ((window.scene && typeof window.scene.lineNum !== "undefined") ? window.scene.lineNum : 0),
              indent: (baseSave && typeof baseSave.indent !== "undefined") ? baseSave.indent : ((window.scene && typeof window.scene.indent !== "undefined") ? window.scene.indent : 0),
              sceneName: safeStats.sceneName || (baseSave && baseSave.sceneName) || ((window.scene && window.scene.sceneName) || ""),
              visited: safeStats.visited || {},
            };
            try { localStorage.setItem("state" + slotName, JSON.stringify(blob)); } catch (e) { console.warn("localStorage blob write failed:", e); }
          } catch (err) {
            console.error("Failed to build/write fallback blob:", err);
            alert("Save failed (fallback). See console.");
            if (typeof done === "function") done();
            return;
          }

          // persist metadata list
          saves.push({ id: id, name: saveName || "", time: id, slot: slotName });
          saveMetaList();
          alert("Game saved (fallback).");
          try { overlay.remove(); } catch (e) {}
          if (typeof setButtonTitles === "function") setButtonTitles();
          if (typeof done === "function") done();
        });
        return;
      } catch (e) {
        console.warn("restoreObject during fallback failed:", e);
        // fall through to use window.stats/window.scene underneath
      }
    }

    // ff we reach here, plugin helpers not available or restoreObject threw; use globals
    try {
      const baseScene = window.scene || {};
      const stateObj = window.stats || {};
      const safeStats = deepCloneSerializable(stateObj) || {};
      const safeTemps = (baseScene && baseScene.temps) ? deepCloneSerializable(baseScene.temps) || {} : {};
      if (typeof safeStats.sceneName === "undefined" && typeof stateObj.sceneName !== "undefined") safeStats.sceneName = stateObj.sceneName;
      const blob = {
        stats: safeStats,
        temps: safeTemps,
        lineNum: (baseScene && typeof baseScene.lineNum !== "undefined") ? baseScene.lineNum : 0,
        indent: (baseScene && typeof baseScene.indent !== "undefined") ? baseScene.indent : 0,
        sceneName: safeStats.sceneName || (baseScene && baseScene.sceneName) || "",
        visited: safeStats.visited || {},
      };
      try { localStorage.setItem("state" + slotName, JSON.stringify(blob)); } catch (e) { console.warn("localStorage blob write failed:", e); }
      ...
      console.error("Fallback manual save failed:", e);
      alert("Save failed (fallback). See console.");
      try { overlay.remove(); } catch (e) {}
      if (typeof setButtonTitles === "function") setButtonTitles();
      if (typeof done === "function") done();
  ...
  /* does new save idk why it freaks out on the very first scene */
  btnRow.appendChild(makeBtn("New Save", function() {
    if (inFlight) return;
    inFlight = true;
    // disable row buttons visually while in-flight
    btnRow.querySelectorAll('button').forEach(b => b.disabled = true);

    const wrapperOverlay = document.querySelector(".csssp-popup-overlay");
    if (wrapperOverlay) { wrapperOverlay.style.pointerEvents = "none"; wrapperOverlay.style.opacity = "0.6"; }

    openSavePrompt(function(saveName) {
      if (wrapperOverlay) { wrapperOverlay.style.pointerEvents = "auto"; wrapperOverlay.style.opacity = "1"; }

      // if user cancelled prompt (openSavePrompt uses false to indicate cancel)
      if (saveName === false || typeof saveName === "undefined") {
      ...
      try {
        const hasSaveCookie = (typeof saveCookie === "function");
        const hasInitStore = (typeof initStore === "function");
        const hasRestoreObject = (typeof restoreObject === "function");

        if (hasInitStore && hasRestoreObject && hasSaveCookie) {
          try {
            restoreObject(initStore(), "state", null, function(baseSave) {
              if (!baseSave || !baseSave.stats) {
                console.warn("restoreObject returned no baseSave; falling back to manual blob.");
                performFallbackManualSave(saveName, () => {
                  btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
                  inFlight = false;
                });
                return;
              ...
              // ensure baseSave.stats exists/serializable
              baseSave.stats = baseSave.stats || {};
              try { baseSave.stats["_smSaveName"] = saveName || ""; baseSave.stats["_smSaveDateId"] = id; } catch (e) { console.warn(e); }

              attemptPluginSave(
                slotName,
                baseSave.stats,
                baseSave.temps || {},
                typeof baseSave.lineNum !== "undefined" ? baseSave.lineNum : (typeof baseSave.line !== "undefined" ? baseSave.line : 0),
                typeof baseSave.indent !== "undefined" ? baseSave.indent : 0,
                function onPluginSuccess() {
                  console.log("Plugin save succeeded:", slotName);
                  setTimeout(function() {
                    saves.push({ id: id, name: saveName || "", time: id, slot: slotName });
                    saveMetaList();
                    alert("Game saved!");
                ...
                function onPluginFailure(err) {
                  console.warn("Plugin save failed; falling back to manual blob:", err);
                  performFallbackManualSave(saveName, () => {
                    btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
                    inFlight = false;
                  });
            ...
            console.warn("restoreObject attempt threw; will fallback:", e);
            performFallbackManualSave(saveName, () => {
              btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
              inFlight = false;
            });
        ...
        // fallback if plugin prerequisites not present
        performFallbackManualSave(saveName, () => {
          btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
          inFlight = false;
        });
  ...
  /* load function */
  function validateAndLoadSlot(slotName, onFound, onMissing) {
    const canonical = canonicalSlot(slotName);
    const maxAttempts = 10;
    const delay = 200;
    let tries = 0;

    function checkOnce(cb) {
      if (typeof initStore === "function" && typeof initStore().get === "function") {
        try {
          initStore().get("state" + canonical, function(success, val) {
            if (success && val) return cb(true);
            if (typeof restoreObject === "function") {
              try {
                restoreObject(initStore(), "state" + canonical, null, function(saveData) {
                  if (saveData && saveData.stats && typeof saveData.lineNum !== "undefined") return cb(true);
                  return cb(false);
                });
                return;
              } catch (e) {
                console.warn("validateAndLoadSlot: restoreObject threw:", e);
                return cb(false);
              }
            }
            return cb(false);
          ...
          console.warn("validateAndLoadSlot: initStore().get threw:", e);
        }
      }
      ...
      if (typeof restoreObject === "function" && typeof initStore === "function") {
        try {
          restoreObject(initStore(), "state" + canonical, null, function(saveData) {
            if (saveData && saveData.stats && (typeof saveData.line !== "undefined" || typeof saveData.lineNum !== "undefined")) return cb(true);
            return cb(false);
          });
          return;
        } catch (e) {
      ...
      try {
        const raw = localStorage.getItem("state" + canonical) || localStorage.getItem(canonical);
        if (!raw) return cb(false);
        try {
          const parsed = JSON.parse(raw);
          if (parsed && parsed.stats && typeof parsed.lineNum !== "undefined") return cb(true);
        } catch (e) {
          return cb(false);
        }
        return cb(false);
      } catch (e) {
        console.warn("validateAndLoadSlot: localStorage check threw:", e);
        return cb(false);
      }
    }
    ...
    (function attempt() {
      tries++;
      checkOnce(function(ok) {
        if (ok) return (typeof onFound === "function") && onFound(canonical);
        if (tries < maxAttempts) {
          setTimeout(attempt, delay);
        } else {
          return (typeof onMissing === "function") && onMissing();
        }
      });
    })();
  }

  btnRow.appendChild(makeBtn("Load", function() {
    if (inFlight) return;
    inFlight = true;
    btnRow.querySelectorAll('button').forEach(b => b.disabled = true);
    ...
    const selectedMeta = saves.find(s => String(s.id) === String(selectEle.value));
    if (!selectedMeta) {
    ...
    const rawSlot = selectedMeta.slot || formatSlotKey(selectedMeta.id);
    const slotName = canonicalSlot(rawSlot);

    if (!confirm("Are you sure you wish to load this save? Current progress will be lost!")) {
      inFlight = false;
      btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
      return;
    }

    validateAndLoadSlot(slotName, function(validSlot) {
      function finalLoad() {
        overlay.classList.add("fade-out");
        setTimeout(() => {
          try { overlay.remove(); } catch(e) {}
          const ctx = (typeof stats !== "undefined" && stats && stats.scene) ? stats.scene : (window.scene || null);
          try {
            if (ctx) {
              clearScreen(loadAndRestoreGame.bind(ctx, validSlot));
            } else {
              clearScreen(function() { loadAndRestoreGame(validSlot); });
            }
          } catch (e) {
            console.error("Error invoking loadAndRestoreGame:", e);
            alert("Failed to load save (internal error).");
          } finally {
            inFlight = false;
            btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
          }
      ...
      if (typeof restoreObject === "function" && typeof initStore === "function") {
        let attempts = 0, maxAttempts = 8, delay = 180;
        (function checkThenDo() {
          attempts++;
          try {
            restoreObject(initStore(), "state" + slotName, null, function(saveData) {
              if (saveData && saveData.stats && typeof saveData.lineNum !== "undefined") return finalLoad();
              if (attempts < maxAttempts) return setTimeout(checkThenDo, delay);
              alert("Failed to read the save from storage. Try again in a moment.");
              inFlight = false;
              btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
            });
          } catch (e) {
            console.warn("Load final restoreObject threw:", e);
            if (attempts < maxAttempts) return setTimeout(checkThenDo, delay);
            alert("Failed to read the save from storage. Try again in a moment.");
            inFlight = false;
            btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
      ...
      if (confirm("Save data missing or corrupted. Remove this entry from the list?")) {
        saves = saves.filter(s => String(s.id) !== String(selectedMeta.id));
        saveMetaList();
        alert("Corrupt save removed.");
        try { overlay.remove(); } catch(e) {}
        if (typeof setButtonTitles === "function") setButtonTitles();
        inFlight = false;
        btnRow.querySelectorAll('button').forEach(b => b.disabled = false);
  ...
  /* delete */
  btnRow.appendChild(makeBtn("Delete", function() {
    if (inFlight) return;
    inFlight = true;
    btnRow.querySelectorAll('button').forEach(b => b.disabled = true);
    ...
    const idToRemove = String(selectEle.value);
    let removed = false, removedMeta = null;
    saves = saves.filter(s => {
      if (String(s.id) === idToRemove) { removed = true; removedMeta = s; return false; }
      return true;
    });
    if (!removed) { alert("Save not found."); inFlight = false; btnRow.querySelectorAll('button').forEach(b => b.disabled = false); return; }
    saveMetaList();

    try {
      const rawSlot = removedMeta && removedMeta.slot ? removedMeta.slot : formatSlotKey(idToRemove);
      const slotName = canonicalSlot(rawSlot);
      if (typeof initStore === "function") {
        try { initStore().remove("state" + slotName, function() {}); } catch (e) { console.warn("initStore().remove failed:", e); }
      } else {
        try { localStorage.removeItem("state" + slotName); } catch (e) {}
      }
    } catch (e) { console.warn("Failed to remove underlying slot:", e); }
  ...
  closeBtn.className = "csssp-popup-close";
  ...
  // enable pointer events after a short delay so CSS animations can run cleanly
  setTimeout(() => { try { overlay.style.pointerEvents = "auto"; } catch (e) {} }, 350);
}



function openSavePrompt(callback) {
  // disable the background overlay (popup) while modal is open
  const mainOverlay = document.querySelector(".csssp-popup-overlay");
  if (mainOverlay) {
    mainOverlay.style.pointerEvents = "none";
    mainOverlay.style.opacity = "0.5";
  }

  // Create modal overlay
  let modal = document.createElement("div");
  modal.className = "csssp-modal-overlay";
  modal.id = "csssp-modal";
  modal.style.zIndex = "100000"; // ensures it's above all popups

  // box
  let box = document.createElement("div");
  box.className = "csssp-modal-box";

  let msg = document.createElement("div");
  msg.className = "csssp-modal-message";
  msg.innerHTML = "Enter a name for your save:";
  box.appendChild(msg);

  let input = document.createElement("input");
  input.className = "csssp-modal-input";
  input.type = "text";
  input.placeholder = "Save name (optional)";
  input.autocomplete = "off";
  box.appendChild(input);

  let btnRow = document.createElement("div");
  btnRow.className = "csssp-modal-buttons";

  let ok = document.createElement("button");
  ok.className = "csssp-modal-ok";
  ok.textContent = "OK";

  let cancel = document.createElement("button");
  cancel.className = "csssp-modal-cancel";
  ...
  // ensure user can press esc even if modal not focused
  box.tabIndex = -1;
  box.focus();
}
...
function showCharacterFiles() {
  ...
  overlay.innerHTML = `
  <!-- DATABASE VIEW (List of Characters) -->
  <div id="databaseView">
    <h1 class="database-title">Character Database</h1>
    <div id="characterList"></div>
    <!-- Add this wrapper to match the style of single-view buttons -->
    <div class="database-view-buttons">
      <button id="databaseReturnToGame">Return to Game</button>
    </div>
  </div>

  <!-- SINGLE CHARACTER VIEW (One Card) -->
  <div id="singleCharacterView" style="display: none;">
    <div id="singleCharacterCardContainer"></div>
    <div class="single-view-buttons">
      <button id="backToDatabaseBtn">Back to Database</button>
      <button id="closeCharacterFiles">Return to Game</button>
    </div>
  </div>
`;
  document.body.appendChild(overlay);
  ...
  // button to return to game from database view
  document.getElementById("databaseReturnToGame").addEventListener("click", returnFromCharacterFiles);

  // use the closeSingleCharacterView function for single-character view buttons
  document.getElementById("backToDatabaseBtn").addEventListener("click", closeSingleCharacterView);
  document.getElementById("closeCharacterFiles").addEventListener("click", closeSingleCharacterView);
}



function returnFromCharacterFiles() {
  console.log("Returning from Character Files to game...");
...
function updateCharacterData() {
  var characterData = document.getElementById("characterData");
  if (!characterData) {
    console.error("Character data element isn't there you gay shit!");
    return;
  }

  console.log("Updating Character Data...");

  var characterCount = parseInt(characterData.getAttribute("data-character-count"), 10) || 2;
  ...
  for (var i = 1; i <= characterCount; i++) {
    var name = characterData.getAttribute(`data-names-${i}`);
    if (name) names.push(name);

    buddyPointsArr.push(parseInt(characterData.getAttribute(`data-buddy-points-${i}`), 10) || 0);
    luvPointsArr.push(parseInt(characterData.getAttribute(`data-luv-points-${i}`), 10) || 0);
    romanceableArr.push(characterData.getAttribute(`data-romanceable-${i}`) === "true");
    lockedArr.push(characterData.getAttribute(`data-locked-${i}`) === "true");
  }

  characters.forEach((char, index) => {
    let cleanTitle = char.title.replace(/^"|"$/g, "").trim();
    let charIndex = names.findIndex(name => name.trim().toLowerCase() === cleanTitle.toLowerCase());

    if (charIndex !== -1) {
      let oldBuddyPoints = char.buddyPoints || 0;
      let oldLuvPoints = char.luvPoints || 0;
      
      let newBuddyPoints = buddyPointsArr[charIndex] || 0;
      let newLuvPoints = luvPointsArr[charIndex] || 0;
      
      // only update buddy points if they have not reached 100
      if (oldBuddyPoints < 100 && newBuddyPoints >= 100) {
        // buddy points have now reached (or exceeded) 100.
        char.buddyPoints = 100; 
        showRankUpMessage(char.title, "Intimacy Unlocked!");
      } else if (oldBuddyPoints >= 100) {
        // buddy points are already locked at or above 100; do not update
      } else {
        // still below 100, should update normally
        char.buddyPoints = newBuddyPoints;
      }
      
      // luv points
      char.luvPoints = newLuvPoints;

      let oldApproval = getApprovalRating(oldBuddyPoints);
      let newApproval = getApprovalRating(newBuddyPoints);

      let oldIntimacy = getIntimacyRating(oldLuvPoints);
      let newIntimacy = getIntimacyRating(newLuvPoints);

      let isRomancing = char.romanceable && newBuddyPoints >= 100;
      if (isRomancing) {
        if (oldApproval !== "Ride or Die") {
          showRankUpMessage(char.title, "Intimacy Unlocked!");
        } else if (oldIntimacy !== newIntimacy) {
          showRankUpMessage(char.title, newIntimacy, true);
        }
      } else if (oldApproval !== newApproval) {
        showRankUpMessage(char.title, newApproval, false);
      }

      char.romanceable = romanceableArr[charIndex];
      characterLocked[char.title] = lockedArr[charIndex];

      char.approvalRating = getApprovalRating(char.buddyPoints);
      char.intimacyRating = isRomancing ? getIntimacyRating(char.luvPoints) : "N/A";
      if (typeof char.previousApprovalRating === "undefined") {
        char.previousApprovalRating = char.approvalRating;
      }
      if (typeof char.previousIntimacyRating === "undefined") {
        char.previousIntimacyRating = char.intimacyRating;
      }
    }
  });

  console.log("Updated Characters Array:", JSON.stringify(characters, null, 2));
...
var characters = [
  {
    title: "\"The American\"",
    realName: "Johnny Steele",
    occupation: "Morningstar Agent",
    gender: "Male",
    appearance: "A 17-year-old American with unkempt blond hair and brown eyes, standing at 5’9”. Typically wears a dress shirt with the collar propped up, the top buttons undone, and a loose yellow tie.",
    snippet: "\"A loudmouthed romantic who flirts with every pretty face. He’s the heart of the group, even if his courage wavers when things get tough.\"",
    ability: "[ROCKETMAN]: Allows him to propel himself at speeds up to Mach 10 using thrusters that manifest across his body. He can detach and launch his limbs as explosive projectiles, with both explosion power and regeneration rate depending on the size of the limb. However, if a limb is severed rather than detached, it cannot regenerate.",
    rarity: "confidential",
    approvalRating: "Acquaintance",
    previousApprovalRating: "Acquaintance",
    romanceable: true,
    luvPoints: 0,
    intimacyRating: "Getting Close",
    previousIntimacyRating: "Getting Close",
    isMain: true, 
    colorScheme: "american-theme" // not active yet
  },

  {
    title: "\"The God Among Men\"",
    realName: "Adam 'Ace' Warbringer",
    occupation: "Morningstar Agent",
    gender: "Male",
    appearance: "A 6’6 striking, dark-skinned, muscular man with a short black hair, scars, an eyepatch, and heart problems. While working as an agent, he sports a dark red jacket draped over his shoulders with no shirt, a tall stiff collar, red stained jeans, and black dress shoes. White bandages, cover his arms and palms of his hands.",
    snippet: "\"A living legend feared by villains and revered by all. A lone hero who won’t stop fighting—even as time chips away at his strength.\"",
    ability: "[IRON TITAN]: A Mutation that makes the user seem indestructible. By greatly increasing their body’s density, they can endure and push through almost any attack—whether physical, elemental, or even temporal. However, the user still feels all pain, and the strain of maintaining this state slowly destroys their body from within.",
    rarity: "top-secret",
    buddyPoints: 0,
    approvalRating: "Acquaintance",
    previousApprovalRating: "Acquaintance", 
...
var characterLocked = {
  "\"The American\"": true,
  "\"The God Among Men\"": true
};

// relationship stages
const relationshipStages = [0, 20, 40, 60, 80, 100];


function getApprovalRating(bp) {
  if (bp >= 100) return "Ride or Die";
  if (bp >= 80) return "Day-One";
  if (bp >= 60) return "Best Friend";
  if (bp >= 40) return "Friend";
  return "Acquaintance";
}

function getIntimacyRating(luvPoints) {
  if (luvPoints >= 100) return "Soulmates";
  if (luvPoints >= 80) return "Going Steady";
  if (luvPoints >= 60) return "Infatuated";
  if (luvPoints >= 40) return "Flirty";
  return "Getting Close";
}

function getApprovalClass(rating) {
  switch (rating) {
    case "Acquaintance":
      return "approval-acquaintance";
    case "Friend":
      return "approval-friend";
    case "Best Friend":
      return "approval-best-friend";
    case "Day-One":
      return "approval-day-one";
    case "Ride or Die":
      return "approval-ride-or-die";
    default:
      return "approval-acquaintance";
  }
}

function getIntimacyClass(rating) {
  switch (rating) {
    case "Getting Close":
      return "intimacy-getting-close";
    case "Flirty":
      return "intimacy-flirty";
    case "Infatuated":
      return "intimacy-infatuated";
    case "Going Steady":
      return "intimacy-going-steady";
    case "Soulmates":
      return "intimacy-soulmates";
    default:
      return "intimacy-getting-close";
  }
}

// get bar width percentage per stage
function getStagePercentage(points) {
  if (points >= 100) return 100; // ensure full bar at max points
  let percentage = 0;
  for (let i = 1; i < relationshipStages.length; i++) {
    if (points < relationshipStages[i]) {
      let lowerBound = relationshipStages[i - 1];
      let upperBound = relationshipStages[i];
      let progress = (points - lowerBound) / (upperBound - lowerBound);
      percentage = ((i - 1) + progress) / (relationshipStages.length - 1) * 100;
      break;
    }
  }
  return percentage;
...
// ensure rnak up message exists
function createRankUpMessageContainer() {
  let existingMessage = document.getElementById("rankUpMessage");
  if (existingMessage) return;

  let rankUpMessage = document.createElement("div");
  rankUpMessage.id = "rankUpMessage";
  rankUpMessage.style.cssText = `
      position: fixed;
      top: 50px;
      ...
      box-shadow: inset -4px -4px 0 rgba(255, 255, 255, 0.8), inset 4px 4px 0 rgba(255, 255, 255, 0.8);
      border-radius: 8px;
      display: flex;
      align-items: center;
  ...
  rankUpMessage.appendChild(rankMeter);
  document.body.appendChild(rankUpMessage);
}



function showRankUpMessage(characterName, newRank, isIntimacy = false) {
  createRankUpMessageContainer();

  let message = document.getElementById("rankUpMessage");

  if (newRank === "Intimacy Unlocked!") {
    message.innerHTML = `${characterName.toUpperCase()} HAS ENTERED THE INTIMACY PHASE!`;
  } else if (isIntimacy) {
    message.innerHTML = `${characterName.toUpperCase()} INTIMACY RANK UP: ${newRank}!`;
  } else {
    message.innerHTML = `${characterName.toUpperCase()} APPROVAL RANK UP: ${newRank}!`;
  }
...
// check for rank-up and display message
function checkRankUp(character, oldPoints, newPoints) {
  let oldRank = getApprovalRating(oldPoints);
  let newRank = getApprovalRating(newPoints);

  if (oldRank !== newRank) {
      showRankUpMessage(character.title, newRank);
  }
}

// update Rank Bar and Show Rank Up Message
function updateRankBar(character, isIntimacy) {
  let points = isIntimacy ? character.luvPoints : character.buddyPoints;
  let previousRank = isIntimacy ? character.prevIntimacyRank : character.prevApprovalRank;
  let currentRank = isIntimacy ? getIntimacyRating(points) : getApprovalRating(points);

  let bar = document.getElementById(`${character.title.replace(/\s/g, '')}-rank-bar`);

  if (bar) {
      bar.style.width = `${getStagePercentage(points)}%`;
  } else {
      console.warn("Rank bar element not found for", character.title);
  }

  if (currentRank !== previousRank) {
      console.log(`${character.title} ranked up to ${currentRank}!`); // debug log
      showRankUpMessage(character.title, currentRank);
      
      if (isIntimacy) {
          character.prevIntimacyRank = currentRank;
      } else {
          character.prevApprovalRank = currentRank;
      }
  }
}
...
/*database list */
function displayDatabaseList() {
  console.log("Displaying database list...");
  var list = document.getElementById("characterList");
  if (!list) {
    console.warn("#characterList not found in DOM.");
    return;
  }
  ...
  characters.forEach((char, index) => {
    var isLocked = characterLocked[char.title] || false;
    var rarityLabel = char.rarity.replace("-", " ").toUpperCase();
    var displayName = isLocked ? "???" : char.title; 

    var item = document.createElement("div");
    item.className = "character-list-item";
    item.innerHTML = `
      <span class="list-name">${displayName}</span>
      <span class="list-rarity">(${rarityLabel})</span>
    `;
    item.addEventListener("click", function() {
      console.log(`Clicked character index ${index}:`, char);
      openSingleCharacterView(index);
    });
...
/*single card view */
function openSingleCharacterView(index) {
  console.log("openSingleCharacterView called with index:", index);

  var dbView = document.getElementById("databaseView");
  if (dbView) dbView.style.display = "none";


  var singleView = document.getElementById("singleCharacterView");
  if (singleView) singleView.style.display = "block";


  displaySingleCharacterCard(index);


  var container = document.getElementById("singleCharacterCardContainer");
  if (!container) {
    console.warn("#singleCharacterCardContainer not found in DOM.");
    return;
  }
  

  const cardEl = container.querySelector(".character-card");
  if (cardEl) {
...
function closeSingleCharacterView() {
  const container = document.getElementById("singleCharacterCardContainer");
  if (!container) return;
  ...
  if (cardEl) {
    ...
    cardEl.addEventListener("transitionend", function handler(e) {
      if (
        ...
        const singleView = document.getElementById("singleCharacterView");
...
function displaySingleCharacterCard(index) {
  console.log("displaySingleCharacterCard called with index:", index);
  ...
  if (index < 0 || index >= characters.length) {
    container.innerHTML = "<p>Error: Character index out of range.</p>";
    return;
  }
  ...
  var rarityClass = rarityMap[char.rarity] || "unclassified";
  var rarityLabel = char.rarity.replace("-", " ").toUpperCase();

  var approval = getApprovalRating(char.buddyPoints);
  var intimacy = getIntimacyRating(char.luvPoints);

  //if buddy points are 100 or more, show romance mode
  var isRomanced = char.romanceable && char.buddyPoints >= 100;
  var romanceBanner = isRomanced
    ? `<div class="romance-banner"><span>♥ ROMANCING ♥</span></div>`
    : "";
  // hide the approval bar if the character is romanced
  var hideApprovalBar = isRomanced;
  var approvalBarHTML = hideApprovalBar ? "" : generateSegmentedBar(char.buddyPoints, "approval");
  // once romanced, only the intimacy (luv) points bar is relevant
  var intimacyBarHTML = isRomanced ? generateSegmentedBar(char.luvPoints, "intimacy") : "";

  var relationshipHTML = isLocked
    ? `<p class="locked-message">[CHARACTER HAS NOT BEEN UNLOCKED YET]</p>`
    : `
      <div class="relationship-bar-container">
        ${!hideApprovalBar ? `
          <div class="approval-section">
            <h3 class="approval-header">Approval Rating</h3>
            <p class="approval-label">[${approval}]</p>
            <div class="relationship-bar approval-bar">${approvalBarHTML}</div>
          </div>` : ""}
        ${isRomanced ? `
          <div class="intimacy-section">
            <h3 class="intimacy-header">Intimacy Rating</h3>
            <p class="intimacy-label">[${intimacy}]</p>
            <div class="relationship-bar intimacy-bar">${intimacyBarHTML}</div>
          </div>` : ""}
      </div>
    `;

  var organizationHTML = char.organization
    ? `<p class="org"><strong>Organization:</strong> ${char.organization}</p>`
    : "";
  var idNumberHTML = char.idNumber
    ? `<p><strong>ID #:</strong> ${char.idNumber}</p>`
    : "";

  var mainClass = char.isMain ? " main-character" : "";
  var customThemeClass = char.colorScheme ? ` ${char.colorScheme}` : "";
  var goldStarHTML = char.isMain
    ? `<div class="gold-star-indicator" title="Main Character">★</div>`
    : "";

  var cardHTML = `
    <div class="character-card id-card${mainClass} ${rarityClass}${customThemeClass}">
      ${goldStarHTML}  <!-- Gold star indicator for main characters -->
      ${romanceBanner}
      <div class="rarity-tab ${rarityClass}">${rarityLabel}</div>
      <div class="id-card-header">
        <h1 class="card-real-name">${isLocked ? "???" : char.realName}</h1>
        <h2 class="card-title">${isLocked ? "???" : char.title}</h2>
      </div>
      ${organizationHTML}
      <div class="id-card-body">
        <div class="id-card-photo">
          <!-- <img src="char.photoURL" alt="Character Photo" /> -->
        </div>
        <div class="id-card-info">
          ${idNumberHTML}
          <p><strong>Occupation:</strong> ${isLocked ? "???" : char.occupation}</p>
          <p><strong>Gender:</strong> ${isLocked ? "???" : char.gender}</p>
          <p><strong>Appearance:</strong> ${isLocked ? "???" : char.appearance}</p>
          <p><strong>Pheromone Ability:</strong> ${isLocked ? "???" : char.ability}</p>
        </div>
      </div>
      <div class="id-card-snippet">
        <p>${isLocked ? "???" : char.snippet}</p>
      </div>
  ...
  console.log("Single card HTML:", cardHTML);
...
function generateSegmentedBar(points, type) {
  const segments = 5;
  const thresholds = [0, 20, 40, 60, 80, 100];

  let activeSegment = 0;
  let segmentFillPercent = 100; // default full segment


  for (let i = 0; i < segments; i++) {
    if (points >= thresholds[i] && (i === segments - 1 || points < thresholds[i + 1])) {
      activeSegment = i;
      if (i < segments - 1 || points < 100) {
        let lowerBound = thresholds[i];
        let upperBound = thresholds[i + 1] || 100;
        segmentFillPercent = ((points - lowerBound) / (upperBound - lowerBound)) * 100;
      }
      break;
    }
  }
  ...
  if (type === "approval") {
    const approvalColors = ["#d0f0c0", "#a8e6a1", "#7cd67c", "#4caf50", "#2e8b2e"];
    let barHTML = "";
    for (let i = 0; i < segments; i++) {
      let fillStyle = "";
      if (i === activeSegment) {
        // the active (partially filled) segment uses a gradient
        fillStyle = `style="background: linear-gradient(to right, ${approvalColors[i]} ${segmentFillPercent}%, #ccc ${segmentFillPercent}%);"`;
      } else if (i < activeSegment) {
        // fully filled segments use their designated color
        fillStyle = `style="background-color: ${approvalColors[i]};"`;
      } else {
        // empty segments use a grey color
        fillStyle = `style="background-color: #ccc;"`;
      }
      barHTML += `<div class="bar-segment" data-segment="${i + 1}" ${fillStyle}></div>`;
    }
    return barHTML;

  } else if (type === "intimacy") {
    const intimacyColors = ["#ffe6f2", "#ffccd9", "#ffb3c0", "#ff99a6", "#ff808c"];
    let heartsHTML = "";
    ...
    for (let i = 0; i < segments; i++) {
      if (i < activeSegment) {
        // fileled
        heartsHTML += `
          <svg class="heart heart-filled" width="24" height="24" viewBox="0 0 24 24">
            <path 
              d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 
                 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 
                 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78
                 -3.4 6.86-8.55 11.54L12 21.35z"
              fill="${intimacyColors[i]}"
              stroke="${intimacyColors[i]}"
              stroke-width="2"
            ></path>
          </svg>
        `;
        ...
        // empty
        heartsHTML += `
          <svg class="heart heart-empty" width="24" height="24" viewBox="0 0 24 24">
            <path 
              d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 
                 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 
                 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78
...
function showMenu() {
  ...
  function menu() {
    ...
    var button = document.getElementById("menuButton");
    button.innerHTML = "Return";
    options = [{name:"Return.", group:"choice", resume:true}];
    ...
    printOptions([""], options, function(option) {
      if (option.resume) {
        ...
        return clearScreen(function() {
          ...
          // clear overlay if it's open
          var overlay = document.getElementById("characterOverlay");
          if (overlay) {
            document.body.removeChild(overlay);
          }
        
          // reset all characters' buddy points, love points, and locked status
          characters.forEach(char => {
            char.buddyPoints = 0;
            char.luvPoints = 0;
            char.approvalRating = "Acquaintance";
            char.intimacyRating = "N/A";
            characterLocked[char.title] = true; // reset locked state
          });
...
function setButtonTitles() {
  ...
  if (button) {
    button.innerHTML = "Agent Profile";
  }
  button = document.getElementById("psychProfileButton");
  if (button) {
    button.innerHTML = "Psych Profile";
  }
  button = document.getElementById("saveSystemButton");
  if (button) {
    button.innerHTML = "Save Menu";
  }
  button = document.getElementById("characterFilesButton");
  if (button) {
    button.innerHTML = "Character Files";
  }
...
function textOptionsMenu(categories) {
  ...
  clearScreen(function() {
    ...
    options = [
      {name:"Return.", group:"choice", resume:true},
    ];
    if (categories.size) {
      options.push(
...
function printFooter() {
  ...
  if (statsButton) {
    if (window.stats && stats.scene && stats.scene.secondaryMode == "stats") {
      statsButton.innerHTML = "Return";
    } else {
      statsButton.innerHTML = "Agent Profile";
...
function fixChromeLinks() {
  ...
  var psychProfileButton = document.getElementById("psychProfile");
  if (psychProfileButton) {
    psychProfileButton.onclick = undefined;
    psychProfileButtonn.addEventListener("click", function() {
      showPsychProfile();
    }, false);
  }

  var saveSystemButton = document.getElementById("saveSystemButton");
  if (saveSystemButton) {
    saveSystemButton.onclick = undefined;
    saveSystemButton.addEventListener("click", function() {
      showSaveSystem();
    }, false);
  }

  var characterFilesButton = document.getElementById("characterFilesButton");
  if (characterFilesButton) {
    characterFilesButton.onclick = undefined;
    characterFilesButton.addEventListener("click", function() {
      showCharacterFiles()
    }, false);
  }
...
document.addEventListener("DOMContentLoaded", function () {
  document.body.addEventListener("click", function (event) {
    if (event.target.id === "closeCharacterFiles") {
      returnFromCharacterFiles();
    }
  });
});